...
* 이 단계에서는 ht−1과 xt를 받아서 0과 1 사이의 값을 Ct−1에 보내줍니다.
...
* Language Model을 예로 들자면, Cell State에는 성별 정보를 가지고 있을 수 있어서 다음 단어의 성별에 맞는대명사를 준비하고 있을 수도 있습니다.
...
* 이전 Cell에서 넘어온 ht-1과 이번 Cell의 입력인 xt를 tanh를 통과시켜 새로운 C~t를 만들고, 동일한 값을 Sigmoid를 통과시켜 어느 정도를 Cell State에 추가할지 정합니다.
...
* 이전 Cell State 값인 Ct-1을 Update해서 새로운 Ct를 만듭니다. 

* 우선 ft값을 곱해서 Ct-1을 조절(잊어버리기)합니다. 그리고, 새롭게 추가되어할 값(Input Gate Layer)을 더해주어 새로운 정보를 추가하는 역할을 합니다.
...
* 그림에서 알 수 있듯이, 이전 Cell의 출력값 ht-1과 현재 Cell의 입력값 xt를 Sigmoid한 다음 새로운 Cell State ct값을 tanh를 통과시켜 -1 ~ 1사이의 값으로 받은 다음에 Sigmoid 값과 곱해서 출력으로 뽑아냅니다.
